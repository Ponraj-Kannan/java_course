...
const CODES = {
  java: [
    ...
    ['c_init', '        int number = sc.nextInt();'], ['', ''],
    ['c_if', '        if (number % 2 == 0) {'],
    ['c_true', '            System.out.println(number + " is Even.");'],
    ['c_else_hdr', '        } else {'],
    ['c_else', '            System.out.println(number + " is Odd.");'],
    ['', '        }'],
    ['c_exit', '    }'], ['', '}'],
  ],
  c: [
    ...
    ['c_main', 'int main() {'],
    ['c_scan', '    int number;'],
    ['c_init', '    scanf("%d", &number);'], ['', ''],
    ['c_if', '    if (number % 2 == 0) {'],
    ['c_true', '        printf("%d is Even.\\n", number);'],
    ['c_else_hdr', '    } else {'],
    ['c_else', '        printf("%d is Odd.\\n", number);'],
    ['', '    }'], ['', ''],
    ['c_exit', '    return 0;'], ['', '}'],
  ],
  cpp: [
    ...
    ['c_init', '    cin >> number;'], ['', ''],
    ['c_if', '    if (number % 2 == 0) {'],
    ['c_true', '        cout << number << " is Even." << endl;'],
    ['c_else_hdr', '    } else {'],
    ['c_else', '        cout << number << " is Odd." << endl;'],
    ['', '    }'], ['', ''],
    ['c_exit', '    return 0;'], ['', '}'],
  ],
  python: [
    ['c_init', 'number = int(input())'], ['', ''],
    ['c_if', 'if number % 2 == 0:'],
    ['c_true', '    print(f"{number} is Even.")'],
    ['c_else_hdr', 'else:'],
    ['c_else', '    print(f"{number} is Odd.")'],
  ],
};

// ── flowchart geometry (wide balanced 2D if-else layout) ──────────
const EDGES = [
  { id: 'e_start_init', d: 'M 270,38 L 270,52' },
  { id: 'e_init_cond', d: 'M 270,86 L 270,104' },
  { id: 'e_cond_yes', d: 'M 165,132 L 115,132 L 115,195' },
  { id: 'e_cond_no', d: 'M 375,132 L 425,132 L 425,195' },
  { id: 'e_true_end', d: 'M 115,233 L 115,260 L 270,260 L 270,285' },
  { id: 'e_false_end', d: 'M 425,233 L 425,260 L 270,260 L 270,285' },
];
...
function buildSteps(numVal) {
  const steps = [];
  const nV = '' + numVal;
  const rem = ((numVal % 2) + 2) % 2; // handle negative numbers gracefully
  const isEven = rem === 0;

  // 1. Enter main()
  steps.push({
    num: null, out: [], curVal: null,
    stage: 'start', edge: null,
    badge: 'main() → program execution starts',
    code: 'c_main',
    vars: [frame('main()', [['number', '—']])],
  });
  ...
  // 3. Read number
  steps.push({
    num: numVal, out: [], curVal: null,
    stage: 'init', edge: 'e_start_init',
    badge: 'int number = sc.nextInt() → read number = ' + numVal,
    code: 'c_init',
    vars: [frame('main()', [['number', nV]])],
  });

  // 4. Test condition: number % 2 == 0
  steps.push({
    num: numVal, out: [], curVal: null,
    stage: 'cond', edge: 'e_init_cond',
    badge: 'if (number % 2 == 0) → ' + numVal + ' % 2 == 0 (' + (isEven ? 'true → take Yes branch' : 'false → take No/else branch') + ')',
    code: 'c_if',
    vars: [frame('main()', [
      ['number', nV],
      ['number % 2', '' + rem],
      ['number % 2 == 0', isEven ? 'true' : 'false', true]
    ])],
  });

  if (isEven) {
    // 5a. True Branch: execute if block
    const outStr = numVal + ' is Even.';
    steps.push({
      num: numVal, out: [outStr], curVal: outStr,
      stage: 'body', edge: 'e_cond_yes',
      badge: 'Condition is true → execute if block: print("' + outStr + '")',
      code: 'c_true',
      vars: [frame('main()', [['number', nV], ['number % 2 == 0', 'true', true]])],
    });

    // 6a. End terminal
    steps.push({
      num: numVal, out: [outStr], curVal: null,
      stage: 'end', edge: 'e_true_end',
      badge: 'Execution moves to End.',
    ...
    steps.push({
      num: numVal, out: [], curVal: null,
      stage: 'else_block', edge: 'e_cond_no',
      badge: 'Condition is false → control jumps to else block',
      code: 'c_else_hdr',
      vars: [frame('main()', [['number', nV], ['number % 2 == 0', 'false', true]])],
    });

    // 5c. False Branch: execute else block
    const outStr = numVal + ' is Odd.';
    steps.push({
      num: numVal, out: [outStr], curVal: outStr,
      stage: 'else_block', edge: 'e_cond_no',
      badge: 'Execute else block: print("' + outStr + '")',
      code: 'c_else',
      vars: [frame('main()', [['number', nV], ['number % 2 == 0', 'false', true]])],
    });
...
const stepsData = reactive(buildSteps(17));
...
function applyInput() {
  const nVal = parseInt(inpNumber.value, 10);
  if (isNaN(nVal)) {
    alert('Please enter a valid integer for number.');
    return;
  }
  playing.value = false;
  inpNumber.value = nVal;
  const built = buildSteps(nVal);
...
<template>
  <div class="slide-wrapper">
    ...
    <div class="ll-toolbar">
      <label>Number</label>
      <input type="number" v-model="inpNumber" class="ll-num-input" />
      ...
      <div class="ll-left-col" ref="leftColRef" :style="{ width: leftWidth + '%' }">
        ...
        <div class="ll-viz-wrap" :style="{ height: vizHeight + 'px' }">
          <div class="ll-perm-area">
            <div class="ll-ptrs">
              <div class="ll-ptr-chip">number = <b class="ll-c-blue">{{ fmt(s.num) }}</b></div>
              <div v-if="s.num !== null" class="ll-ptr-chip">
                number % 2 == 0 : <b :class="s.num % 2 === 0 ? 'll-c-green' : 'll-c-purple'">{{ s.num % 2 === 0 ? 'true (Even)' : 'false (Odd)' }}</b>
              </div>
            </div>

            <!-- FLOWCHART -->
            <div class="ll-flow-wrap">
              <svg viewBox="0 0 540 330" class="fc-svg" xmlns="http://www.w3.org/2000/svg">
                <defs>
                  <marker id="ie-arrow" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse">
                    <path d="M 0 1.5 L 9 5 L 0 8.5 z" class="fc-arrowhead" />
                  </marker>
                  <marker id="ie-arrow-active" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="7" markerHeight="7" orient="auto-start-reverse">
                ...
                <!-- Center: Init number = sc.nextInt() -->
                <g class="fc-node node-init" :class="{ 'fc-node-active': s.stage === 'init' }">
                  <rect x="195" y="52" width="150" height="34" rx="7" class="fc-shape fc-shape-init" />
                  <text x="270" y="68" class="fc-label font-mono">number = {{ inpNumber }}</text>
                  <text x="270" y="79" class="fc-sublabel">read input</text>
                </g>

                <!-- Center: Decision Diamond: number % 2 == 0 ? -->
                <g class="fc-node node-cond" :class="{ 'fc-node-active': s.stage === 'cond' }">
                  ...
                  <text x="270" y="128" class="fc-label fc-label-sm font-mono font-bold">number % 2 == 0 ?</text>
                  <text x="270" y="142" class="fc-sublabel">evaluate condition</text>
                </g>

                <!-- Left Branch (Yes): print(number + " is Even.") -->
                <g class="fc-node node-body" :class="{ 'fc-node-active': s.stage === 'body' }">
                  ...
                  <text x="115" y="211" class="fc-label font-mono font-bold">print(number + " is Even.")</text>
                  <text x="115" y="223" class="fc-sublabel">execute if block</text>
                </g>

                <!-- Right Branch (No): print(number + " is Odd.") -->
                <g class="fc-node node-else" :class="{ 'fc-node-active': s.stage === 'else_block' }">
                  <rect x="330" y="195" width="190" height="38" rx="7" class="fc-shape fc-shape-else" />
                  <text x="425" y="211" class="fc-label font-mono font-bold">print(number + " is Odd.")</text>
                  <text x="425" y="223" class="fc-sublabel">execute else block</text>
                </g>

                <!-- Center: End terminal -->
                <g class="fc-node node-end" :class="{ 'fc-node-active': s.stage === 'end' }">
                  <rect x="225" y="285" width="90" height="30" rx="15" class="fc-shape fc-shape-term" />
                  <text x="270" y="304" class="fc-label">End</text>
                </g>
                ...
                <path
                  ...
                  :marker-end="s.edge === edge.id ? 'url(#ie-arrow-active)' : 'url(#ie-arrow)'"
                />
                ...
                <!-- Annotations -->
                <text x="65" y="165" class="fc-loopback-hint">Even path</text>
                <text x="475" y="165" class="fc-loopback-hint">Odd path</text>
              </svg>
            </div>
