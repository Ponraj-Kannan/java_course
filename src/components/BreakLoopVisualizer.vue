...
const CODES = {
  java: [
    ...
    ['c_breakAt', '        int breakAt = sc.nextInt();'], ['', ''],
    ['c_for_init', '        for (int i = 1; i <= end; i++) {'],
    ['c_if', '            if (i == breakAt) {'],
    ['c_break', '                break; // exit loop immediately'],
    ['', '            }'],
  ...
  c: [
    ['', '#include <stdio.h>'], ['', ''],
    ['c_main', 'int main() {'],
    ['c_scan', '    int end, breakAt;'],
    [['c_end', 'c_breakAt'], '    scanf("%d %d", &end, &breakAt);'], ['', ''],
    ['c_for_init', '    for (int i = 1; i <= end; i++) {'],
    ['c_if', '        if (i == breakAt) {'],
    ['c_break', '            break;'],
    ['', '        }'],
  ...
  cpp: [
    ...
    ['c_main', 'int main() {'],
    ['c_scan', '    int end, breakAt;'],
    [['c_end', 'c_breakAt'], '    cin >> end >> breakAt;'], ['', ''],
    ['c_for_init', '    for (int i = 1; i <= end; i++) {'],
    ['c_if', '        if (i == breakAt) {'],
    ['c_break', '            break;'],
    ['', '        }'],
...
const EDGES = [
  ...
  { id: 'e_break_yes', d: 'M 275,164 L 275,227 L 145,227' },
  { id: 'e_break_no', d: 'M 340,140 L 385,140' },
...
function buildSteps(endVal, breakVal) {
  const steps = [];
  const eV = '' + endVal;
  const bV = '' + breakVal;
  const baseRows = (iVal) => [
  ...
  steps.push({
    ...
    code: 'c_end',
    vars: [frame('main()', [['end', eV], ['breakAt', '—'], ['i', '—']])],
  });
  ...
  steps.push({
    i: null, out: [], curVal: null,
    stage: 'start', edge: null,
    badge: 'int breakAt = sc.nextInt() → read breakAt = ' + breakVal,
    code: 'c_breakAt',
  ...
  while (true) {
    ...
    // 7. Check if (i == breakAt)
    const isBreak = (i === breakVal);
    steps.push({
      i, out: out.slice(), curVal: null,
      stage: 'break_cond', edge: 'e_cond_yes',
      badge: 'Check: if (i == breakAt) → ' + i + ' == ' + breakVal + ' (' + (isBreak ? 'true → trigger break' : 'false → proceed to body') + ')',
      code: 'c_if',
      vars: [frame('main()', baseRows(i).concat([['i == breakAt', isBreak ? 'true' : 'false', true]]))],
    });
    ...
    if (isBreak) {
      ...
      steps.push({
        i, out: out.slice(), curVal: null,
        stage: 'break_hit', edge: 'e_break_yes',
        badge: '⚡ break; triggered! Exiting the loop immediately at i = ' + i,
        code: 'c_break',
        vars: [frame('main()', baseRows(i))],
      });

      // 9. Reach End via break
      steps.push({
        i, out: out.slice(), curVal: null,
        stage: 'end', edge: 'e_break_yes',
        badge: 'Exited loop due to break statement → reached End',
        code: 'c_exit', done: true,
        vars: [frame('main()', baseRows(i))],
      });
      break;
...
function applyInput() {
  ...
  const bVal = parseInt(inpBreakAt.value, 10);
  if (isNaN(eVal) || isNaN(bVal) || eVal < 1) {
    alert('Enter valid whole numbers for End and Break at.');
    return;
  }
  playing.value = false;
  inpEnd.value = eVal;
  inpBreakAt.value = bVal;
  const built = buildSteps(eVal, bVal);
...
<template>
  <div class="slide-wrapper">
    ...
    <div class="ll-toolbar">
      ...
      <input type="number" v-model="inpBreakAt" class="ll-num-input" min="1" max="20" />
        ...
        <div class="ll-viz-wrap" :style="{ height: vizHeight + 'px' }">
          <div class="ll-perm-area">
            <div class="ll-ptrs">
              ...
              <div class="ll-ptr-chip">breakAt = <b class="ll-c-purple">{{ inpBreakAt }}</b></div>
            ...
            <div class="ll-flow-wrap">
              <svg viewBox="0 0 540 270" class="fc-svg" xmlns="http://www.w3.org/2000/svg">
                <defs>
                  <marker id="bkfc-arrow" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse">
                    <path d="M 0 1.5 L 9 5 L 0 8.5 z" class="fc-arrowhead" />
                  </marker>
                  <marker id="bkfc-arrow-active" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="7" markerHeight="7" orient="auto-start-reverse">
                ...
                <!-- Middle Column: Break Condition: i == breakAt ? -->
                <g class="fc-node node-break-cond" :class="{ 'fc-node-active': s.stage === 'break_cond' || s.stage === 'break_hit' }">
                  <polygon points="275,116  340,140  275,164  210,140" class="fc-shape fc-shape-break" />
                  <text x="275" y="137" class="fc-label fc-label-sm font-mono font-bold">i == breakAt ?</text>
                  <text x="275" y="149" class="fc-sublabel">check break</text>
                </g>
                ...
                <g class="fc-node node-incr" :class="{ 'fc-node-active': s.stage === 'incr' }">
                  <rect x="385" y="185" width="110" height="34" rx="7" class="fc-shape fc-shape-incr" />
                  <text x="440" y="201" class="fc-label font-mono font-bold">i++</text>
                  <text x="440" y="212" class="fc-sublabel">increment</text>
                </g>
                ...
                <path
                  ...
                  :marker-end="s.edge === edge.id ? 'url(#bkfc-arrow-active)' : 'url(#bkfc-arrow)'"
                />
                ...
                <g transform="translate(282, 176)">
                  <rect x="0" y="0" width="46" height="16" rx="4" class="fc-badge-break" />
                  <text x="23" y="11" class="fc-badge-break-text">break</text>
                </g>
                ...
                <!-- Annotations -->
                <text x="210" y="221" class="fc-loopback-hint">⚡ break exit</text>
                <text x="350" y="248" class="fc-loopback-hint">◄ repeat loop</text>
              </svg>
            </div>
...
.fc-node.node-break-cond.fc-node-active .fc-shape { fill: #fee2e2; stroke: #ef4444; }
