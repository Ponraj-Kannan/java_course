...
const CODES = {
  java: [
    ...
    ['c_forhdr', '        for (int i = start; i <= end; i++) {'],
  ...
  python: [
    ...
    ['c_end', 'end = int(input())'], ['', ''],
    ['c_forhdr', 'for i in range(start, end + 1):'],
    ['c_body', '    print(i, end=" ")'],
  ],
};

// ── flowchart geometry (fixed layout, independent of data) ────────
const EDGES = [
  { id: 'e_start_init', d: 'M 105,46 L 105,68' },
  { id: 'e_init_cond', d: 'M 105,106 L 105,122' },
  { id: 'e_cond_yes', d: 'M 175,162 L 235,162' },
  { id: 'e_process_update', d: 'M 295,182 L 295,212' },
  { id: 'e_update_cond', d: 'M 235,232 L 16,232 L 16,162 L 35,162' },
  { id: 'e_cond_no', d: 'M 105,202 L 105,286' },
];
...
function buildSteps(start, end) {
  ...
  steps.push({
    ...
    badge: 'for (int i = ' + start + '; ...) → initialize loop variable i = ' + i, code: 'c_forhdr',
  ...
  while (true) {
    ...
    steps.push({
      i, out: out.slice(), curVal: condTrue ? i : null,
      stage: 'cond', edge: firstCond ? 'e_init_cond' : 'e_update_cond',
      badge: 'check condition: i (' + i + ') <= end (' + end + ') → ' + (condTrue ? 'true → enter loop body' : 'false → exit loop'),
      code: 'c_forhdr',
    ...
    if (!condTrue) {
      steps.push({
        ...
        badge: 'loop finished → control reaches End', code: 'c_exit', done: true,
        vars: [frame('main()', baseRows(i))],
      });
      break;
    }
    ...
    out = [...out, i];
    steps.push({
      i, out: out.slice(), curVal: i, stage: 'body', edge: 'e_cond_yes',
      badge: 'print(i) → output ' + i, code: 'c_body',
      vars: [frame('main()', baseRows(i))],
    });

    const next = i + 1;
    steps.push({
      i: next, out: out.slice(), curVal: null, stage: 'incr', edge: 'e_process_update',
      badge: 'i++ → i becomes ' + next, code: 'c_forhdr',
...
const activeEdgePath = computed(() => EDGE_PATHS[s.value.edge] || '');
        ...
        <div class="ll-viz-wrap" :style="{ height: vizHeight + 'px' }">
          <div class="ll-perm-area">
            ...
            <div class="ll-flow-wrap">
              <svg viewBox="0 0 400 340" class="fc-svg" xmlns="http://www.w3.org/2000/svg">
                <defs>
                  <marker id="ffc-arrow" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse">
                    <path d="M 0 1.5 L 9 5 L 0 8.5 z" class="fc-arrowhead" />
                  </marker>
                  <marker id="ffc-arrow-active" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="7" markerHeight="7" orient="auto-start-reverse">
                ...
                <g class="fc-node node-start" :class="{ 'fc-node-active': s.stage === 'start' }">
                  <rect x="55" y="14" width="100" height="32" rx="16" class="fc-shape fc-shape-term" />
                  <text x="105" y="34" class="fc-label">Start</text>
                </g>

                <!-- Init -->
                <g class="fc-node node-init" :class="{ 'fc-node-active': s.stage === 'init' }">
                  <rect x="50" y="68" width="110" height="38" rx="7" class="fc-shape fc-shape-init" />
                  <text x="105" y="86" class="fc-label font-mono">i = start</text>
                  <text x="105" y="98" class="fc-sublabel">initialize</text>
                </g>

                <!-- Decision -->
                <g class="fc-node node-cond" :class="{ 'fc-node-active': s.stage === 'cond' }">
                  <polygon points="105,122  175,162  105,202  35,162" class="fc-shape fc-shape-cond" />
                  <text x="105" y="159" class="fc-label fc-label-sm font-mono font-bold">i &lt;= end ?</text>
                  <text x="105" y="172" class="fc-sublabel">test condition</text>
                </g>

                <!-- Process -->
                <g class="fc-node node-body" :class="{ 'fc-node-active': s.stage === 'body' }">
                  <rect x="235" y="142" width="120" height="40" rx="7" class="fc-shape fc-shape-body" />
                  <text x="295" y="161" class="fc-label font-mono font-bold">print(i)</text>
                  <text x="295" y="173" class="fc-sublabel">loop body</text>
                </g>

                <!-- Update -->
                <g class="fc-node node-incr" :class="{ 'fc-node-active': s.stage === 'incr' }">
                  <rect x="235" y="212" width="120" height="40" rx="7" class="fc-shape fc-shape-incr" />
                  <text x="295" y="231" class="fc-label font-mono font-bold">i++</text>
                  <text x="295" y="243" class="fc-sublabel">increment</text>
                </g>

                <!-- End terminal -->
                <g class="fc-node node-end" :class="{ 'fc-node-active': s.stage === 'end' }">
                  <rect x="55" y="286" width="100" height="32" rx="16" class="fc-shape fc-shape-term" />
                  <text x="105" y="306" class="fc-label">End</text>
                </g>
                ...
                <path
                  ...
                  :marker-end="s.edge === edge.id ? 'url(#ffc-arrow-active)' : 'url(#ffc-arrow)'"
                />

                <!-- branch badge labels -->
                <g transform="translate(182, 137)">
                  <rect x="0" y="0" width="36" height="18" rx="4" class="fc-badge-yes" />
                  <text x="18" y="13" class="fc-badge-yes-text">Yes</text>
                </g>

                <g transform="translate(112, 238)">
                  <rect x="0" y="0" width="32" height="18" rx="4" class="fc-badge-no" />
                  <text x="16" y="13" class="fc-badge-no-text">No</text>
                </g>

                <!-- Loop-back annotation -->
                <text x="125" y="226" class="fc-loopback-hint">◄ loop back to condition</text>
              </svg>
            </div>
