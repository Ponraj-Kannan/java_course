...
const EDGES = [
  { id: 'e_start_init', d: 'M 110,40 L 110,56' },
  { id: 'e_init_cond', d: 'M 110,90 L 110,116' },
  { id: 'e_cond_no', d: 'M 110,164 L 110,212' },
  { id: 'e_cond_yes', d: 'M 175,140 L 285,140' },
  { id: 'e_process_update', d: 'M 340,157 L 340,185' },
  { id: 'e_update_cond', d: 'M 340,219 L 340,252 L 20,252 L 20,140 L 45,140' },
];
...
function buildSteps(start, end) {
  ...
  // 5. Initialize i = start before while loop
  let i = start;
  steps.push({
  ...
  while (true) {
    ...
    steps.push({
      ...
      badge: 'while (i <= end) → check condition: ' + i + ' <= ' + end + ' (' + (condTrue ? 'true → enter loop body' : 'false → exit loop') + ')',
      code: 'c_whilehdr',
    ...
    if (!condTrue) {
      steps.push({
        ...
        badge: 'condition is false → control reaches End', code: 'c_exit', done: true,
        vars: [frame('main()', baseRows(i))],
      });
      break;
    }
    ...
    steps.push({
      ...
      badge: 'i++ → increment counter to ' + next, code: 'c_incr',
        ...
        <div class="ll-viz-wrap" :style="{ height: vizHeight + 'px' }">
          <div class="ll-perm-area">
            ...
            <div class="ll-flow-wrap">
              <svg viewBox="0 0 460 270" class="fc-svg" xmlns="http://www.w3.org/2000/svg">
                <defs>
                  ...
                  <marker id="wfc-arrow-active" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="7" markerHeight="7" orient="auto-start-reverse">
                    <path d="M 0 0.5 L 9 5 L 0 9.5 z" class="fc-arrowhead-active" />
                  </marker>
                </defs>

                <!-- Left Column: Start terminal -->
                <g class="fc-node node-start" :class="{ 'fc-node-active': s.stage === 'start' }">
                  <rect x="65" y="12" width="90" height="28" rx="14" class="fc-shape fc-shape-term" />
                  <text x="110" y="29" class="fc-label">Start</text>
                </g>

                <!-- Left Column: Init i = start -->
                <g class="fc-node node-init" :class="{ 'fc-node-active': s.stage === 'init' }">
                  <rect x="55" y="56" width="110" height="34" rx="7" class="fc-shape fc-shape-init" />
                  <text x="110" y="72" class="fc-label font-mono">i = start</text>
                  <text x="110" y="83" class="fc-sublabel">initialize</text>
                </g>

                <!-- Left Column: Condition i <= end -->
                <g class="fc-node node-cond" :class="{ 'fc-node-active': s.stage === 'cond' }">
                  <polygon points="110,116  175,140  110,164  45,140" class="fc-shape fc-shape-cond" />
                  <text x="110" y="137" class="fc-label fc-label-sm font-mono font-bold">i &lt;= end ?</text>
                  <text x="110" y="149" class="fc-sublabel">test condition</text>
                </g>

                <!-- Left Column: End terminal -->
                <g class="fc-node node-end" :class="{ 'fc-node-active': s.stage === 'end' }">
                  <rect x="65" y="212" width="90" height="30" rx="15" class="fc-shape fc-shape-term" />
                  <text x="110" y="231" class="fc-label">End</text>
                </g>

                <!-- Right Column: Loop Body: print(i) -->
                <g class="fc-node node-body" :class="{ 'fc-node-active': s.stage === 'body' }">
                  <rect x="285" y="123" width="110" height="34" rx="7" class="fc-shape fc-shape-body" />
                  <text x="340" y="139" class="fc-label font-mono font-bold">print(i)</text>
                  <text x="340" y="150" class="fc-sublabel">loop body</text>
                </g>

                <!-- Right Column: Increment: i++ -->
                <g class="fc-node node-incr" :class="{ 'fc-node-active': s.stage === 'incr' }">
                  <rect x="285" y="185" width="110" height="34" rx="7" class="fc-shape fc-shape-incr" />
                  <text x="340" y="201" class="fc-label font-mono font-bold">i++</text>
                  <text x="340" y="212" class="fc-sublabel">increment</text>
                </g>
                ...
                <path
                  ...
                  :marker-end="s.edge === edge.id ? 'url(#wfc-arrow-active)' : 'url(#wfc-arrow)'"
                />
                ...
                <text x="240" y="248" class="fc-loopback-hint">◄ loop back to condition</text>
              </svg>
            </div>
